...
class Utils
{
    public static function autoload($className)
    {
        ...
        if ((empty($path[0]) || $path[0] != 'Phresto') && (empty($path[1]) || $path[1] != 'Modules')) {
            return;
        }
        ...
        foreach ($modules as $module => $files) {
            ...
            } elseif (!empty($files['class']) && in_array($path[2] . '.php', $files['class'])) {
    ...
    public static function updateModules()
    {
        $getFiles = function ($base, $flag = 0) {
            return array_map(function ($elem) use ($base, $flag) {
                return str_replace($base, '', $elem);
            }, glob($base . '*', $flag));
        };
